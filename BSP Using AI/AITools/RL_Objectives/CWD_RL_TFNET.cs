    ...
    public class CWD_RL_TFNET
    {
        ...
        public void Fit(string modelName, List<Sample> trainingSamplesList, long datasetSize, long modelId)
        {
            ...
            if (_aiBackThreadReportHolderForAIToolsForm != null)
                _aiBackThreadReportHolderForAIToolsForm.holdAIReport(new object[] { "progress", modelName, 1, 1 }, "AIToolsForm");
            ...
            if (_aiBackThreadReportHolderForAIToolsForm != null)
                _aiBackThreadReportHolderForAIToolsForm.holdAIReport(new object[] { "fitting_complete", modelName, datasetSize }, "AIToolsForm");
        }
        ...
        {
            TFNETReinforcementL model = new TFNETReinforcementL() { Name = name, BaseModel = new TFNETBaseModel(path) };

            model.BaseModel.Session = createTFNETNeuralNetModelSession(inputDim, outputDim);

            // Save the model
            if (path.Length > 0)
                TF_NET_NN.SaveModelVariables(model.BaseModel.Session, path);
