...
{
    public partial class ValidationDataSelectionForm : Form
    {
        ...
        public ValidationDataSelectionForm(DataTable dataTable, ValidateModel validateModel)
        {
            InitializeComponent();
        ...
        public void ReloadData()
        {
            ...
            foreach (DataRow row in _dataList)
            {
                ...
                ValidationDataItemUserControl validationDataItemUserControl = new ValidationDataItemUserControl(SetSignalOrder, _dataList.Count);
        ...
        private void SetSignalOrder(ValidationDataItemUserControl item, int newOrderIndex)
        {
            ...
            if (movementDirection > 0)
                ...
                for (int i = newOrderIndex; i < currentOrderIndex; i++)
                    CopyItemToIndex(i, i + 1);
            ((ValidationDataItemUserControl)signalsFlowLayoutPanel.Controls[currentOrderIndex]).categoryLabel.Text = itemDataCategory;
            ((ValidationDataItemUserControl)signalsFlowLayoutPanel.Controls[currentOrderIndex])._ignoreEvent = true;
            ((ValidationDataItemUserControl)signalsFlowLayoutPanel.Controls[currentOrderIndex]).orderTextBox.Text = currentOrderIndex.ToString();
            ((ValidationDataItemUserControl)signalsFlowLayoutPanel.Controls[currentOrderIndex])._ignoreEvent = false;
        ...
        {
            ((ValidationDataItemUserControl)signalsFlowLayoutPanel.Controls[newIndex]).categoryLabel.Text = ((ValidationDataItemUserControl)signalsFlowLayoutPanel.Controls[itemIndex]).categoryLabel.Text;
            ((ValidationDataItemUserControl)signalsFlowLayoutPanel.Controls[newIndex])._ignoreEvent = true;
            ((ValidationDataItemUserControl)signalsFlowLayoutPanel.Controls[newIndex]).orderTextBox.Text = ((ValidationDataItemUserControl)signalsFlowLayoutPanel.Controls[itemIndex]).orderTextBox.Text;
            ((ValidationDataItemUserControl)signalsFlowLayoutPanel.Controls[newIndex])._ignoreEvent = false;
        }
        ...
        private void startValidationButton_Click(object sender, EventArgs e)
        {
            ...
            if (holdoutValidationRadioButton.Checked)
            {
                ...
                for (int i = 0; i < _dataList.Count; i++)
                    ...
                    if (i < _trainingSetCapacity)
                        modelData.TrainingData.Add((ARTHTFeatures)Garage.ByteArrayToObject(_dataList[i].Field<byte[]>("features")));
                    else
                        modelData.ValidationData.Add((ARTHTFeatures)Garage.ByteArrayToObject(_dataList[i].Field<byte[]>("features")));
                data.Add(modelData);
            ...
            else if (crossValidationRadioButton.Checked)
            {
                ...
                for (int i = 0; i < _dataList.Count; i++)
                {
                    ...
                    dataFolds[selectedFold].Add((ARTHTFeatures)Garage.ByteArrayToObject(_dataList[i].Field<byte[]>("features")));
                }
